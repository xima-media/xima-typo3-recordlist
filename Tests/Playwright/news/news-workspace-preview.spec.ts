...
// Sentence the fixtures put in the draft bodytext only
const DRAFT_MARKER = 'Draft revision awaiting review.';
...
async function expectDraftIsRendered(page: Page, uri: string): Promise<void> {
  const preview = await page.context().newPage();
  try {
    await preview.goto(uri);
    await expect(preview.locator('body')).toContainText(DRAFT_MARKER);
  } finally {
    await preview.close();
  }
}
...
test.describe('News Workspace Preview', () => {
  ...
  test('every workspace draft is previewed in the frontend', async ({ page }) => {
    ...
    for (const originalUid of DRAFT_ORIGINAL_UIDS) {
      ...
      await expectDraftIsRendered(page, uri);
    }
  });

  test('the view button of the editing form previews the draft', async ({ page }) => {
    const contentFrame = await openModule(page, 'example_news');

    await contentFrame.locator(`tr[data-t3ver_oid="${DRAFT_ORIGINAL_UIDS[0]}"] a[aria-label="Edit"]`).first().click();
    await contentFrame.locator('.module-docheader').first().waitFor({ timeout: 5000 });

    const uri = await contentFrame.locator('a.t3js-editform-view').first().getAttribute('href') as string;

    expect(uri).not.toContain('/typo3/workspace/preview-control');
    expect(uri).toContain('ADMCMD_prev=IGNORE');
    expect(uri).toContain('workspaceId=1');
